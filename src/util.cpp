...
const char * const BITCOIN_CONF_FILENAME = "karimuncoin.conf";
const char * const BITCOIN_PID_FILENAME = "karimuncoind.pid";
...
#else
    const char* pszModule = "karimuncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Karimuncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Karimuncoin";
#else
    // Unix
    return pathRet / ".karimuncoin";
#endif
#endif
}
