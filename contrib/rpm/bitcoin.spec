...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17266
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17267
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27266
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19335
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 37267
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 37268
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 17266
	%{_sbindir}/semanage port -d -p tcp 17267
	%{_sbindir}/semanage port -d -p tcp 27266
	%{_sbindir}/semanage port -d -p tcp 19335
	%{_sbindir}/semanage port -d -p tcp 37267
	%{_sbindir}/semanage port -d -p tcp 37268
	for selinuxvariant in %{selinux_variants}; do
